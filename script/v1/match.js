        ...
        case CONSTANTS.TEAM1:
        {
            for(var i = 0; i < this.teamB_.Players.length; ++i)
                if((Math.abs(x - this.teamB_.Players[i].GetMidX()) < distance)
                    ...
                    && (!(this.teamB_.Players[i].HasState(FLAGS.INVULNERABLE)))
        ...
        case CONSTANTS.TEAM2:
        {
            for(var i = 0; i < this.teamA_.Players.length; ++i)
                if((Math.abs(x - this.teamA_.Players[i].GetMidX()) < distance)
                    ...
                    && (!(this.teamA_.Players[i].HasState(FLAGS.INVULNERABLE)))
...
Match.prototype.Start = function(team1,team2)
{
    ...
    var endAttack           = function(thisValue,players) { return function(id) { for(var i = 0; i < players.length;++i) { this.RemoveState(FLAGS.CAN_BE_BLOCKED); players[i].SetAllowBlock(id,thisValue.GetGame().GetCurrentFrame(),false); } } }
    ...
    var endAirAttack        = function(thisValue,players) { return function(id) { for(var i = 0; i < players.length;++i) { this.RemoveState(FLAGS.CAN_BE_AIR_BLOCKED); players[i].SetAllowAirBlock(id,thisValue.GetGame().GetCurrentFrame(),false); } } }
