...
{
    if(!!(otherFlags & FLAGS.IGNORE_COLLISIONS))
        return false;
    return true;
}
...
Player.prototype.SetAllowBlock = function(attackId,frame,isAllowed,x,y)
{
    if(!!isAllowed)
    {
        ...
        this.AddPoseFlags(FLAGS.ALLOW_BLOCK);
    }
    else
    {
...
Player.prototype.SetAllowAirBlock = function(attackId,frame,isAllowed,x,y)
{
    if(!!isAllowed)
    {
        ...
        /*allow block*/
        this.AddPoseFlags(FLAGS.ALLOW_AIR_BLOCK);
    ...
    else
    {
        ...
        if(this.blockedAirAttacks_.length == 0)
            this.RemovePoseFlags(FLAGS.ALLOW_AIR_BLOCK);
        /*air attacks can also be blocked on the ground*/
        this.SetAllowBlock(attackId,frame,isAllowed,x,y);
    }
}
...
Player.prototype.SetRegisteredHit = function(attackState,hitState,flags,frame,damage,energyToAdd,isProjectile,hitX,hitY,attackDirection,who,hitID,moveOverrideFlags,otherPlayer,fx,fy,behaviorFlags,invokedAnimationName)
{
    ...
    if(!!isProjectile && !!this.currentAnimation_.Animation && !!(this.currentAnimation_.Animation.state_ & FLAGS.IGNORE_PROJECTILES))
        return false;
    return true;
}
...
{
    return this.HasState(FLAGS.BLOCKING) && this.HasPoseFlags(FLAGS.CROUCHING);
}
Player.prototype.IsBlockingInAir = function()
{
    return this.HasState(FLAGS.BLOCKING) && this.IsAirborne();
}
...
{
    return this.HasState(FLAGS.BLOCKING);
}
...
Player.prototype.TakeHit = function(attackState,hitState,flags,frame,damage,energyToAdd,isProjectile,hitX,hitY,attackDirection,who,hitID,moveOverrideFlags,fx,fy,otherPlayer,behaviorFlags,invokedAnimationName)
{
    ...
    if(!!(attackState & ATTACK_FLAGS.THROW_START))
    {
        ...
        if(this.IsBlocking())
        {
            ...
            this.RemovePoseFlags(FLAGS.ALLOW_BLOCK);
            /*if the player is blocking, then remove it*/
            this.RemoveState(FLAGS.BLOCKING);
        }
    ...
    else
    {
        ...
        {
            if(!!(attackState & ATTACK_FLAGS.TRIP)) {move = this.moves_[_c3("_",FLAGS.STANDING|FLAGS.CROUCHING,"_hr_trip")];}
            if(!!(attackState & ATTACK_FLAGS.LIGHT)) {slideAmount = CONSTANTS.DEFAULT_CROUCH_LIGHT_HRSLIDE; move = this.moves_[_c3("_",FLAGS.CROUCHING,"_hr_cLN")];}
            if(!!(attackState & ATTACK_FLAGS.MEDIUM)) {slideAmount = CONSTANTS.DEFAULT_CROUCH_MEDIUM_HRSLIDE; move = this.moves_[_c3("_",FLAGS.CROUCHING,"_hr_cMN")];}
            if(!!(attackState & ATTACK_FLAGS.HARD)) {slideAmount = CONSTANTS.DEFAULT_CROUCH_HARD_HRSLIDE; move = this.moves_[_c3("_",FLAGS.CROUCHING,"_hr_cHN")];}
        }
        else
        {
            if(!!(attackState & ATTACK_FLAGS.TRIP) && !!(hitState & HIT_FLAGS.NEAR)) {move = this.moves_[_c3("_",FLAGS.STANDING|FLAGS.CROUCHING,"_hr_trip")];}
            else if(!!(attackState & ATTACK_FLAGS.KNOCKDOWN)) {move = this.moves_[_c3("_",FLAGS.STANDING,"_hr_knockdown")];}

            else if(!!(attackState & ATTACK_FLAGS.LIGHT) && !!(hitState & HIT_FLAGS.NEAR)) {slideAmount = CONSTANTS.DEFAULT_LIGHT_HRSLIDE; move = this.moves_[_c3("_",FLAGS.STANDING,"_hr_sLN")];}
            else if(!!(attackState & ATTACK_FLAGS.LIGHT) && !!(hitState & HIT_FLAGS.FAR)) {slideAmount = CONSTANTS.DEFAULT_LIGHT_HRSLIDE; move = this.moves_[_c3("_",FLAGS.STANDING,"_hr_sLF")];}

            else if(!!(attackState & ATTACK_FLAGS.MEDIUM) && !!(hitState & HIT_FLAGS.NEAR)) {slideAmount = CONSTANTS.DEFAULT_MEDIUM_HRSLIDE; move = this.moves_[_c3("_",FLAGS.STANDING,"_hr_sMN")];}
            else if(!!(attackState & ATTACK_FLAGS.MEDIUM) && !!(hitState & HIT_FLAGS.FAR)) {slideAmount = CONSTANTS.DEFAULT_MEDIUM_HRSLIDE; move = this.moves_[_c3("_",FLAGS.STANDING,"_hr_sMF")];}

            else if(!!(attackState & ATTACK_FLAGS.HARD) && !!(hitState & HIT_FLAGS.NEAR)) {slideAmount = CONSTANTS.DEFAULT_HARD_HRSLIDE; move = this.moves_[_c3("_",FLAGS.STANDING,"_hr_sHN")];}
            else if(!!(attackState & ATTACK_FLAGS.HARD) && !!(hitState & HIT_FLAGS.FAR)) {slideAmount = CONSTANTS.DEFAULT_HARD_HRSLIDE; move = this.moves_[_c3("_",FLAGS.STANDING,"_hr_sHF")];}
        }
    }
    ...
    else if(this.HasState(FLAGS.BLOCKING))
    {
        if(!!flags)
            this.SpawnHitReportAnimations(frame, flags, hitState, relAttackDirection);
...
{
    var move = this.moves_[_c3("_",FLAGS.STANDING,"_hr_dead")];
    if(!!move)
    {
        attackDirection = this.GetRelativeDirection(attackDirection);
...
{
    this.RemoveState(FLAGS.DEAD);
    this.TakeDamage(-20);
    var move = this.moves_[_c3("_",FLAGS.NONE,"_getup")];
...
{
    var move = this.moves_[_c3("_",FLAGS.STANDING|FLAGS.CROUCHING,"_hr_trip")];
    if(!!move)
    {
        this.AddState(FLAGS.AIRBORNE);
        var direction = this.GetAttackDirection(attackDirection);
...
{
    var move = this.moves_[_c3("_",FLAGS.STANDING,"_eject")];
    if(!!move)
    {
        //attackDirection = this.GetRelativeDirection(attackDirection);
        var direction = this.GetAttackDirection(attackDirection);
...
{
    var move = this.moves_[_c3("_",FLAGS.STANDING,"_hr_knockdown")];
    if(!!move)
    {
        var direction = this.GetAttackDirection(attackDirection);
        this.SetCurrentAnimation({Animation:move,StartFrame:frame,Direction:this.direction_,AttackDirection:direction});
...
{
    var move = this.moves_[_c3("_",FLAGS.AIRBORNE,"_hr_air")];
    if(!!move)
    {   
        var direction = -this.GetAttackDirection(attackDirection);
        this.SetCurrentAnimation({Animation:move,StartFrame:frame,Direction:this.direction_,AttackDirection:direction});
