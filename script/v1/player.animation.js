    ...
    if(this.HasState(FLAGS.MOBILE))
    {
        ...
        for(var i in this.moves_)
        {
            ...
            if(!!move.energyToSubtract_ && currentEnergy < move.energyToSubtract_)
                continue;
            var pstate = (move.requiredPoseState_ | FLAGS.ALLOW_BLOCK | FLAGS.ALLOW_AIR_BLOCK) ^ (FLAGS.ALLOW_BLOCK | FLAGS.ALLOW_AIR_BLOCK);
            var mustAllowBlock = !!(move.requiredPoseState_ & FLAGS.ALLOW_BLOCK);
            var mustAllowAirBlock = !!(move.requiredPoseState_ & FLAGS.ALLOW_AIR_BLOCK);
            if(!pstate || this.HasPoseFlags(pstate))
            {
            
                if(!!mustAllowBlock && !(this.HasPoseFlags(FLAGS.ALLOW_BLOCK)))
                    continue;
                if(!!mustAllowAirBlock && !(this.HasPoseFlags(FLAGS.ALLOW_AIR_BLOCK)))
                    continue;
...
{
    var frontKey = "";
         if(!!(this.HasState(FLAGS.BLOCKING))) {frontKey = _c3("_",ATTACK_FLAGS.FRONT|ATTACK_FLAGS.BLOCK,"_0");}
    ...
    if(!this.HasState(FLAGS.BLOCKING) && !!(flags & ATTACK_FLAGS.REAR))
    {
        var rearKey = "";
...
{
    if(!!this.currentAnimation_ && !!this.currentAnimation_.Animation && !!(this.currentAnimation_.Animation.state_ & FLAGS.LOOP_IF_KEYDOWN))
    {
...
Player.prototype.SetCurrentAnimation = function(newAnimation)
{
    ...
    if(!!this.currentAnimation_ && this.currentAnimation_.Animation)
    {
        ...
        {
            this.RemoveState(FLAGS.IGNORE_COLLISIONS);
            this.flags_.PlayerFlags.Remove(PLAYER_FLAGS.IGNORE_COLLISIONS);
            this.fixXFn_(1);
        }
    }
    ...
    if(!!newAnimation && !!newAnimation.Animation)
    {
        ...
        this.SetPoseFlags(newAnimation.Animation.poseState_);
        this.flags_.PoseFlags.Set(newAnimation.Animation.poseState_);
        this.adjustShadowPosition_ = newAnimation.Animation.adjustShadowPosition_;
        if(!(newAnimation.Animation.state_ & FLAGS.HOLD_ZINDEX))
            this.MoveToFront();
        this.OffsetImageX(0);
        this.OffsetImageY(0);
...
Player.prototype.SetCurrentFrame = function(newFrame,frame,stageX,stageY,ignoreTranslation)
{
    if(!!this.currentFrame_)
    {
        ...
        if(this.HasState(FLAGS.SMALLER_AABB))
        {
        ...
        this.RemoveFlags(this.currentFrame_.FlagsToSet);
    }
    ...
    if(!!newFrame)
    {
        ...
        if(!this.HasState(FLAGS.PROJECTILE_ACTIVE) && !!(newFrame.FlagsToSet & FLAGS.SPAWN_PROJECTILE))
        {
            /*this.onStartAirAttackFn_();*/
            /*this.onStartAttackFn_();*/
            this.projectiles_[newFrame.chainProjectile_].Throw(frame,stageX,stageY);
        }
        if(!!(newFrame.FlagsToSet & FLAGS.SPAWN_BIGDIRT))
        {
            this.ShowBigDirt(frame);
        }
        else if(!!(newFrame.FlagsToSet & FLAGS.SPAWN_SMALLDIRT))
        {
        ...
        if(!!(newFrame.FlagsToSet & FLAGS.MOVE_TO_BACK) || !!(newFrame.FlagsToSet & FLAGS.BLOCKING))
            this.MoveToBack();
        if(!!(newFrame.FlagsToSet & FLAGS.MOVE_TO_FRONT))
            this.MoveToFront();

        ignoredFlags = FLAGS.AIRBORNE | FLAGS.AIRBORNE_FB; /*flags to be set in the OnFrame function*/
        this.AddState((newFrame.FlagsToSet | ignoredFlags) ^ ignoredFlags);
        if(!!(newFrame.FlagsToSet & FLAGS.AIRBORNE) || !!(newFrame.FlagsToSet & FLAGS.AIRBORNE_FB))
        {
            this.SetPoseFlags((newFrame.FlagsToSet | ignoredFlags) ^ ignoredFlags);
        }
        this.RemoveState(newFrame.FlagsToClear);

        if(!!(newFrame.FlagsToSet & FLAGS.SMALLER_AABB) || !!(newFrame.FlagsToSet & FLAGS.SMALLER_AABB))
        {
