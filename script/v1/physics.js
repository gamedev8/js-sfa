...
Physics.prototype.TryAttack = function(hitDelayFactor,hitID,frame,points,flagsToSend,attackFlags,p1,p2,damage,moveOverrideFlags,energyToAdd,behaviorFlags,invokedAnimationName)
{
    ...
    else if(p2.HasState(FLAGS.INVULNERABLE) && !(behaviorFlags & BEHAVIOR_FLAGS.THROW))
        return;
...
{
    if(p2.HasState(FLAGS.IGNORE_PROJECTILES))
        return;
    ...
    else if(p2.HasState(FLAGS.INVULNERABLE) || !projectile)
        return;
...
Physics.prototype.HasIntersection = function(a, b)
{
...
Physics.prototype.GetPlayersAtPosition = function(rect,who,type)
{
    var team = who.team_;
    var temp = [];
    var retVal = [];
...
Physics.prototype.FixX = function(amount,player,dontOverrideSign,canFixStageX)
{
    if(!!amount)
    {
        ...
        if(amount > 0) /*moving right*/
        {
            ...
            var collisions = this.GetPlayersAtPosition(myRect,player,CONSTANTS.RIGHT_AND_CHECK_LEFT).reverse();
        ...
        else /*moving left*/
        {
            ...
            var collisions = this.GetPlayersAtPosition(myRect,player,CONSTANTS.LEFT_AND_CHECK_RIGHT).reverse();
            for(var i = 0, length = collisions.length; i < length; ++i)
            {
                collisions[i].MoveCircleToTop();
                var otherRect = collisions[i].GetRect();
...
Physics.prototype.MoveX = function(amount,player,dontOverrideSign,canFixStageX,doubleImpededAmount)
{
    if(!!amount)
    {
        ...
        if(amount > 0) /*moving right*/
        {
            ...
            {
                collisions = this.GetPlayersAtPosition(myRect,player,CONSTANTS.LEFT);
                for(var i = 0, length = collisions.length; i < length; ++i)
                {
                    collisions[i].MoveCircleToTop();
                    var otherRect = collisions[i].GetRect();
        ...
        else /*moving left*/
        {
            ...
            {
                collisions = this.GetPlayersAtPosition(myRect,player,CONSTANTS.RIGHT);
                for(var i = 0, length = collisions.length; i < length; ++i)
                {
                    collisions[i].MoveCircleToTop();
                    var otherRect = collisions[i].GetRect();
